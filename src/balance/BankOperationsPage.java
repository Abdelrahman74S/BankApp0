...
public class BankOperationsPage extends JFrame {
	...
	public BankOperationsPage(BankAccount bankAccount) {
	    ...
	    // Add a button to return to Login/Register
	    JButton backButton = new JButton("Back to Login/Register");
	    backButton.setBounds(280 ,370 ,232 ,57);
	    backButton.addActionListener(e -> {  
	        Login loginForm = new Login();  
	        loginForm.setVisible(true);  
	        dispose();  
	    });
	    operationsPane.add(backButton);
	...
	private void setupButtons() {  
	    ...
	    depositButton.setBounds(10 ,25 ,232 ,66);
	    ...
	    withdrawalButton.setBounds(563 ,30 ,232 ,57);
   ...
   private void performLoanRepayment() {  
       String repaymentAmountStr= JOptionPane.showInputDialog(this,"Enter repayment amount (Max due is $"+bankAccount.getOutstandingLoanAmount()+"):");   
       ...
       if(recipientIdStr!= null && !recipientIdStr.isEmpty()) {   
           ...
           if(transferAmountStr!= null && !transferAmountStr.isEmpty()) {   
               try {   
                   ...
                   Optional<User> recipientOpt= Register.users.values().stream().filter(user -> user.getAccountId()== Integer.parseInt(recipientIdStr)).findFirst();

                   if(recipientOpt.isPresent()) {   
                       User recipient= recipientOpt.get();   
                       bankAccount.transfer(recipient.getBankAccount(),transferAmount ,recipient.getUsername());   
                       JOptionPane.showMessageDialog(this,"Transferred: $"+transferAmount+ " to "+recipient.getUsername()+ "\nNew Balance: $"+bankAccount.getBalance());   
                   } else {   
